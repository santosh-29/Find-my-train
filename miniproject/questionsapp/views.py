...
from .forms import AnswerForm, QuestionForm, ProfileForm, RegisterForm
...
def home(request):
    ...
    # print(all_ques)
    return render(request, "home.html", context={"quests": all_ques})
...
def tag(request, tag):
    quests = Question.objects.annotate(total_comments=Count('answer__comment')).filter(tags__icontains=tag).order_by(
        '-id')
